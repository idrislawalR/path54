  ...
  return (
    <section id="sectors" className="bg-background py-[clamp(80px,9vw,132px)]">
      <div className="container-shell">
        ...
        <div className="mt-16 grid border-l border-t border-[#2A2A2A] md:grid-cols-2 xl:grid-cols-4">
          {sectors.map((sector, index) => (
            <article key={sector} className="min-h-[175px] border-b border-r border-[#2A2A2A] p-8">
              <p className="display text-[12px] leading-4 tracking-[1.2px] text-[#AA8147]">
                {String(index + 1).padStart(2, "0")}
              </p>
              <h3 className="display mt-8 max-w-[245px] text-[30px] leading-[36.77px] text-[#F3F0EB]">
                {sector}
              </h3>
            </article>
          ))}
          <article className="min-h-[175px] border-b border-r border-[#2A2A2A] p-8">
